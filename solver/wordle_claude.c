...
static void tt_init(TT* tt, uint64_t capacity_pow2) {
    ...
    for (uint64_t i = 0; i < capacity_pow2; i++) tt->entries[i].hash = TT_EMPTY_HASH;
}
...
// do hit.
static TTEntry* tt_find(TT* tt, uint64_t hash, uint32_t size) {
    uint64_t idx = hash & tt->mask;
    for (uint64_t probes = 0; probes <= tt->mask; probes++) {
        TTEntry* e = &tt->entries[idx];
        if (e->hash == TT_EMPTY_HASH) return NULL;
        if (e->hash == hash && e->size == size) return e;
        idx = (idx + 1) & tt->mask;
    }
    return NULL;
}

// Finds an existing slot for (hash,size), or an empty slot to claim, or
// NULL if the table is degenerately full (never happens in practice --
// table is sized generously and, if it were to fill, we just stop
// caching, which is always safe).
static TTEntry* tt_find_or_claim(TT* tt, uint64_t hash, uint32_t size) {
    uint64_t idx = hash & tt->mask;
    for (uint64_t probes = 0; probes <= tt->mask; probes++) {
        TTEntry* e = &tt->entries[idx];
        if (e->hash == TT_EMPTY_HASH) {
            e->hash = hash;
            e->size = size;
...
typedef struct {
    ...
    // a generation stamp instead of memset (O(1) amortized "clear").
...
static uint32_t solve_subset(Solver* solver, const uint32_t* targets, uint32_t count,
                              uint64_t hash, uint32_t beta, uint32_t* out_guess) {
    ...
    TTEntry* entry = tt_find(&solver->tt, hash, count);
    ...
    if (node_lb >= beta) {
        ...
        TTEntry* e = tt_find_or_claim(&solver->tt, hash, count);
    ...
    for (uint32_t g = 0; g < num_guesses; g++) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            h = (h ^ row[targets[i]]) * 1099511628211ULL;
        }
        uint64_t idx = h & dedup_mask;
        bool duplicate = false;
        while (solver->dedup_stamp[idx] == call_id) {
            if (solver->dedup_hash[idx] == h) { duplicate = true; break; }
    ...
    RankedCandidate ranked[rep_count];
    {
        ...
        for (uint32_t r = 0; r < rep_count; r++) {
            ...
            for (int s = 0; s < NUM_SCORES; s++) {
                if (hist[s] > 0) { active++; sum_sq += hist[s] * hist[s]; }
            }
    ...
    for (uint32_t c = 0; c < rep_count; c++) {
        ...
        for (uint32_t b = 0; b < active_buckets; b++) {
            ...
            uint64_t bh = 0;
            for (uint32_t j = 0; j < buckets[b].size; j++) bh ^= game->zobrist[local_partition[off + j]];
            buckets[b].hash = bh;
        ...
        for (uint32_t b = 0; b < active_buckets; b++) {
            ...
            uint32_t bucket_cost = solve_subset(solver, &local_partition[buckets[b].offset], buckets[b].size,
                                                 buckets[b].hash, bucket_beta, NULL);
...
static void partition_root(GameData* game, uint32_t opener_idx, uint32_t* local_partition,
    ...
    for (int s = 0; s < NUM_SCORES; s++) {
        if (hist[s] > 0) {
            ...
            uint64_t bh = 0;
            for (uint32_t j = 0; j < hist[s]; j++) bh ^= game->zobrist[local_partition[off + j]];
            buckets[active++] = (BucketInfo){ .score = (uint16_t)s, .size = hist[s], .offset = off, .hash = bh };
        }
    }
...
// required for correctness -- each is just solved with beta=UINT32_MAX),
// so this is embarrassingly parallel and introduces no synchronization
// beyond an atomic work index.
// -------------------------------------------------------------
...
static void* bucket_worker(void* arg) {
    ...
    while (1) {
        ...
        uint32_t cost = solve_subset(&solver, &pool->local_partition[bkt->offset], bkt->size,
                                      bkt->hash, UINT32_MAX, &best_guess);
        pool->out_costs[idx] = cost;
        pool->out_guesses[idx] = best_guess;
        pool->out_nodes[idx] = solver.nodes_visited;
    }
    solver_free(&solver);
    return NULL;
}
...
static TreeNode* build_subtree_node(Solver* solver, const uint32_t* targets, uint32_t count, uint64_t hash);

static TreeNode* build_subtree_node_with_guess(Solver* solver, const uint32_t* targets, uint32_t count,
                                                uint64_t hash, uint32_t best_g) {
    GameData* game = solver->game;
    (void)hash;
    ...
    for (int s = 0; s < NUM_SCORES; s++) {
        ...
        if (s == EXACT_MATCH) {
            ...
            uint64_t bh = 0;
            for (uint32_t j = 0; j < hist[s]; j++) bh ^= game->zobrist[local_partition[off + j]];
            node->children[s] = build_subtree_node(solver, &local_partition[off], hist[s], bh);
        }
    }
...
static TreeNode* build_subtree_node(Solver* solver, const uint32_t* targets, uint32_t count, uint64_t hash) {
    ...
    uint32_t best_g;
    solve_subset(solver, targets, count, hash, UINT32_MAX, &best_g);
    return build_subtree_node_with_guess(solver, targets, count, hash, best_g);
}
...
static void* tree_bucket_worker(void* arg) {
    ...
    while (1) {
        ...
        BucketInfo* bkt = &pool->buckets[idx];
        if (bkt->score == EXACT_MATCH) continue;
        pool->out_nodes[idx] = build_subtree_node(&solver, &pool->local_partition[bkt->offset], bkt->size, bkt->hash);
    }
    solver_free(&solver);
    return NULL;
}
