...
### Verified raw scoring benchmarks (Apple M5, 4P+6E cores, `uv run python benchmark_scoring.py`)

| Scoring Mode | 14.8k x 2.3k Matrix (Round 1 Pool) | 14.8k x 14.8k Matrix (Full Pool) | Throughput | Speedup vs Original |
| :--- | :--- | :--- | :--- | :--- |
| **Scalar Python** (`scoring.get_score`) | ~38 s (extrapolated) | ~283 s (extrapolated) | 0.66 M pairs/sec | 1x |
| **NumPy Vectorized** | 1.89 s | 12.5 s | 17.5-18 M pairs/sec | ~27x |
| **C Accelerated** (Multi-threaded) | **0.04 s** | **0.25 s** | **~850-900 M pairs/sec** | **~50x (vs NumPy) / ~1,300x (vs scalar)** |
| **Fused C Score + Bincount** | **0.04 s** | -- | **~750-850 M pairs/sec** | Fused 1-Pass |
...
### Net effect (measured, `benchmark_strategies.py --sample-size 100`)
