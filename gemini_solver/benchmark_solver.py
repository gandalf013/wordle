#!/usr/bin/env python3
"""Benchmark harness for wordle_gemini.c vs wordle_claude.c.
"""
...
DEFAULT_BINARY = SOLVER_DIR / "wordle_gemini"
CLAUDE_BINARY = REPO_ROOT / "solver" / "wordle_claude"
...
def compare(old_label, new_label):
    ...
    if not correctness_ok:
        print(
            ...
            "Per design goals, any change here is a correctness\n"
...
def main(argv=None):
    ...
    parser.add_argument("--build", action="store_true", help="run `make -C gemini_solver` before benchmarking")
    parser.add_argument("--regen-wordlists", action="store_true", help="rebuild fixture word lists from words.txt")
    parser.add_argument(
        "--cases",
        nargs="+",
        choices=[*FIXTURES, "full"],
        default=list(FIXTURES),
        help="which fixtures to run (default: %(default)s)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="solver thread count (default: %(default)s)",
    )
    parser.add_argument("--label", default=None, help="tag for this run's results")
    parser.add_argument("--compare", nargs=2, metavar=("OLD_LABEL", "NEW_LABEL"), help="diff two labels' results")
